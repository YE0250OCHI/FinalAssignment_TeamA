...
||Main|`Task`|プログラム起動処理と各タスクの呼び出し|
||SendOnlineAsync|`Task`|オンライン通知|
||HandleAlarmAsync|`Task`|異常処理|
|WaitPickingReq|||出庫要求待機のタスク|
||ExecuteAsync|`Task`|出庫要求待機|
||UnavailableAsync|`Task`|対応不可レスポンス処理|
|PollingPicking|||出庫指示問合せのタスク|
||ExecuteAsync|`Task`|出庫指示問合せ|
||AddJobAsync|`Task`|出庫処理バッファへのデータ格納|
|WaitStoringReq|||入庫要求待機のタスク|
||ExecuteAsync|`Task`|入庫要求待機|
|PickingTask|||出庫処理のタスク|
||ExecuteAsync|`Task`|出庫処理|
